...
**Reading the snapshot via mcpScript.** TemPad returns the full code,
tokens, and assets in `structuredContent`, but pi's direct-tool call only
surfaces the short `content` summary to the model — pi-mcp-adapter drops
`structuredContent` from model-facing text per the MCP SDK contract (content
wins when both are present). To read the real design data, run the reusable
script at `.agents/scripts/tempad-dev-get-code.mjs` via `mcpScript`. Apply
per-call overrides inline in the code you pass to `mcpScript` (`nodeId`,
`preferredLang`, `resolveTokens`, `vectorMode`); edit the file's CONFIG block
only to change permanent defaults. The script calls
`tools.call('tempad-dev_get_code', …)`, reads `data.structuredContent`, and
returns it as the model-facing result. Use this path instead of the plain
direct-tool call whenever the full code, tokens, or assets are needed.
...
**Reading structure via mcpScript.** `get_structure` has the same
content-vs-structuredContent split as `get_code`: the real outline is in
`structuredContent`, which pi's direct-tool call drops from model-facing text.
Run the reusable script at `.agents/scripts/tempad-dev-get-structure.mjs` via
`mcpScript`. Apply per-call overrides inline in the code you pass to
`mcpScript` (`nodeId`, `depth`); edit the file's CONFIG block only to change
permanent defaults. The script reads `data.structuredContent` and returns the
`roots` tree — hierarchy and geometry only, no styles/tokens/assets. Use the
returned node `id` values as `nodeId` for narrower `get_code` follow-ups.
